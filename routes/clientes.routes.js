...
module.exports = ({
  ...
  router.get('/:empresa', auth, requirePermissao(pool, 'clientes', 'ver'), async (req, res, next) => {
    ...
    try {
      ...
      if (busca) {
        ...
        sql += `
          AND (
            LOWER(COALESCE(c.nome, '')) LIKE $${idx} ESCAPE '\\'
            OR LOWER(COALESCE(c.cpf, '')) LIKE $${idx} ESCAPE '\\'
            OR LOWER(COALESCE(c.telefone, '')) LIKE $${idx} ESCAPE '\\'
          )
        `;
