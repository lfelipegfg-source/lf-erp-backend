  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO produtos
        (empresa, nome, preco, custo, estoque, estoque_minimo, codigo_barras, categoria, criado_em, atualizado_em)
...
app.put('/produtos/:id', auth, async (req, res) => {
  try {
    ...
    const atualResult = await pool.query(`SELECT * FROM produtos WHERE id = $1 AND empresa = $2`, [
      id,
      empresaResolvida.nome
    ]);
...
app.delete('/produtos/:id', auth, async (req, res) => {
  try {
    ...
    const vendaItemResult = await pool.query(
      `SELECT COUNT(*) AS total FROM venda_itens WHERE produto_id = $1 AND empresa = $2`,
      [id, empresaResolvida.nome]
    );

    const compraItemResult = await pool.query(
      `SELECT COUNT(*) AS total FROM compra_itens WHERE produto_id = $1`,
      [id]
    );
    ...
    if (
      ...
      return res.status(400).send('Produto já possui movimentações e não pode ser excluído');
    }

    await pool.query(`DELETE FROM movimentacoes_estoque WHERE produto_id = $1 AND empresa = $2`, [
      id,
      empresaResolvida.nome
    ]);

    await pool.query(`DELETE FROM produtos WHERE id = $1 AND empresa = $2`, [
      id,
      empresaResolvida.nome
    ]);
...
app.post('/clientes', auth, async (req, res) => {
  try {
    const { empresa, empresa_id, nome, endereco, telefone, nascimento, cpf } = req.body;

    if (!nome) {
      return res.status(400).send('Preencha os campos obrigatórios do cliente');
    }
    ...
    const result = await pool.query(
      `INSERT INTO clientes
        (empresa, nome, endereco, telefone, nascimento, cpf, criado_em, atualizado_em)
        VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
        RETURNING id`,
      [empresaResolvida.nome, nome, endereco || '', telefone || '', nascimento || '', cpf || '']
    );
...
app.get('/clientes/:empresa', auth, async (req, res) => {
  try {
    ...
    let sql = `SELECT * FROM clientes WHERE empresa = $1`;
    const params = [empresaResolvida.nome];
    let idx = 2;
...
app.put('/clientes/:id', auth, async (req, res) => {
  try {
    ...
    const existe = await pool.query(`SELECT * FROM clientes WHERE id = $1 AND empresa = $2`, [
      id,
      empresaResolvida.nome
    ]);
    ...
    await pool.query(
      ...
      [nome, endereco || '', telefone || '', nascimento || '', cpf || '', id, empresaResolvida.nome]
    );
...
app.delete('/clientes/:id', auth, async (req, res) => {
  try {
    ...
    const vendaResult = await pool.query(
      `SELECT COUNT(*) AS total FROM vendas WHERE cliente_id = $1 AND empresa = $2`,
      [id, empresaResolvida.nome]
    );

    if (Number(vendaResult.rows[0].total || 0) > 0) {
      return res.status(400).send('Cliente já possui vendas vinculadas e não pode ser excluído');
    }

    await pool.query(`DELETE FROM clientes WHERE id = $1 AND empresa = $2`, [
      id,
      empresaResolvida.nome
    ]);
...
app.post('/fornecedores', auth, async (req, res) => {
  try {
    const { empresa, empresa_id, nome, cnpj, contato, telefone, email, endereco, observacao } =
      req.body;

    if (!nome) {
      return res.status(400).send('Preencha os campos obrigatórios do fornecedor');
    }
    ...
    const result = await pool.query(
      `INSERT INTO fornecedores
        (empresa, nome, cnpj, telefone, email, endereco, observacao, criado_em, atualizado_em)
...
app.get('/fornecedores/:empresa', auth, async (req, res) => {
  try {
    ...
    let sql = `SELECT * FROM fornecedores WHERE empresa = $1`;
    const params = [empresaResolvida.nome];
    let idx = 2;
...
app.put('/fornecedores/:id', auth, async (req, res) => {
  try {
    ...
    const existe = await pool.query(`SELECT * FROM fornecedores WHERE id = $1 AND empresa = $2`, [
      id,
      empresaResolvida.nome
    ]);
...
app.delete('/fornecedores/:id', auth, async (req, res) => {
  try {
    ...
    const compraResult = await pool.query(
      `SELECT COUNT(*) AS total FROM compras WHERE fornecedor_id = $1 AND empresa = $2`,
      [id, empresaResolvida.nome]
    );
    ...
    if (Number(compraResult.rows[0].total || 0) > 0) {
      return res
        .status(400)
        .send('Fornecedor já possui compras vinculadas e não pode ser excluído');
    }

    await pool.query(`DELETE FROM fornecedores WHERE id = $1 AND empresa = $2`, [
      id,
      empresaResolvida.nome
    ]);
...
app.post('/vendas', auth, async (req, res) => {
  ...
  try {
    if (!podeGerenciarVendas(req)) {
      return res.status(403).send('Sem permissão para vendas');
    }
    ...
    if (!limiteVendas.permitido) {
      return res.status(403).send(limiteVendas.mensagem);
    }
    ...
    if (clienteIdFinal) {
      const clienteResult = await client.query(
        ...
        [clienteIdFinal, empresaResolvida.nome]
      );
      ...
      await client.query(
        `UPDATE produtos
          SET estoque = $1,
              atualizado_em = NOW()
          WHERE id = $2 AND empresa = $3`,
        [estoqueAtual - quantidade, produto.id, empresaResolvida.nome]
      );
    ...
    const pagamentoNormalizado = String(pagamento || '').toLowerCase();
    const statusPg = String(status_pagamento || '').toLowerCase();

    const geraContaReceber =
      pagamentoNormalizado.includes('promiss') ||
      statusPg === 'pendente' ||
      statusPg === 'parcial' ||
      Boolean(conta_receber?.gerar);
    ...
    if (geraContaReceber) {
      await criarParcelasContasReceber({
        ...
        cliente_nome: clienteNomeFinal || 'Cliente não informado',
        total: totalFinal,
        quantidade_parcelas: Math.max(1, normalizarInt(parcelas || 1)),
        data_primeiro_vencimento:
          normalizarDataISO(conta_receber?.data_primeiro_vencimento || data) || hoje(),
        intervalo_dias: Math.max(1, normalizarInt(conta_receber?.intervalo_dias || 30)),
        observacao: conta_receber?.observacao || observacao || '',
...
app.get('/vendas-detalhe/:id', auth, async (req, res) => {
  try {
    const id = Number(req.params.id);
    ...
    const venda = vendaResult.rows[0];
    const empresaResolvida = await validarAcessoEmpresa(req, venda.empresa);
    ...
    const contasReceberResult = await pool.query(
      `SELECT * FROM contas_receber WHERE venda_id = $1 ORDER BY parcela ASC, id ASC`,
      [id]
    );
...
app.get('/estoque/movimentacoes/:empresa', auth, async (req, res) => {
  try {
    const empresaResolvida = await validarAcessoEmpresa(req, empresa);
...
app.post('/estoque/ajuste', auth, async (req, res) => {
  ...
  try {
    ...
    const produtoResult = await client.query(
      `SELECT * FROM produtos WHERE id = $1 AND empresa = $2`,
      [produto_id, empresaResolvida.nome]
    );
    ...
    await client.query(
      `UPDATE produtos
        ...
        WHERE id = $2 AND empresa = $3`,
      [novoEstoque, produto_id, empresaResolvida.nome]
    );
    ...
    await client.query('COMMIT');
    res.json({ sucesso: true, estoque_atual: novoEstoque });
  } catch (error) {
    await client.query('ROLLBACK');
...
app.get('/dashboard', auth, async (req, res) => {
  try {
    ...
    const empresaFinal = await obterNomeEmpresaAcesso(req, empresaInformada);

    if (!empresaFinal) {
      return res.status(403).send('Sem acesso');
    }

    await atualizarStatusContasReceberPorEmpresa(empresaFinal);
    await atualizarStatusContasPagarPorEmpresa(empresaFinal);

    const { dataInicial, dataFinal } = obterPeriodo(req);

    const vendasParams = [empresaFinal];
    const comprasParams = [empresaFinal];
    const receberParams = [empresaFinal];
    const pagarParams = [empresaFinal];
    const clientesParams = [empresaFinal];
    const produtosParams = [empresaFinal];

    let vendasWhere = `WHERE empresa = $1`;
    let comprasWhere = `WHERE empresa = $1`;
    let receberWhere = `WHERE empresa = $1 AND status IN ('pendente', 'atrasado')`;
    let pagarWhere = `WHERE empresa = $1 AND status IN ('pendente', 'atrasado')`;
    let clientesWhere = `WHERE empresa = $1`;
    let produtosWhere = `WHERE empresa = $1`;
    ...
    const topProdutosParams = [empresaFinal];
    let topProdutosJoinAndWhere = `
    FROM venda_itens vi
    INNER JOIN vendas v ON v.id = vi.venda_id AND v.empresa = vi.empresa
    WHERE vi.empresa = $1
  `;
...
app.get('/relatorios/financeiro/resumo/:empresa', auth, async (req, res) => {
  try {
    const empresa = req.params.empresa;
    const empresaResolvida = await validarAcessoEmpresa(req, empresa);
    ...
    const paramsPagar = [empresaResolvida.nome];
    const paramsLanc = [empresaResolvida.nome];
    const paramsFluxoReceber = [empresaResolvida.nome];
    const paramsFluxoPagar = [empresaResolvida.nome];
    const paramsInvest = [empresaResolvida.nome];
    const paramsVendas = [empresaResolvida.nome];
    const paramsCompras = [empresaResolvida.nome];

    let whereReceber = `WHERE empresa = $1`;
    let wherePagar = `WHERE empresa = $1`;
    let whereLanc = `WHERE empresa = $1`;
    let whereFluxoReceber = `WHERE empresa = $1 AND LOWER(COALESCE(status, 'pendente')) = 'pago'`;
    let whereFluxoPagar = `WHERE empresa = $1 AND LOWER(COALESCE(status, 'pendente')) = 'pago'`;
    ...
    res.status(500).send('Erro ao gerar resumo financeiro');
  }
});

app.get('/relatorios/financeiro/contas-receber/:empresa', auth, async (req, res) => {
  try {
    const empresa = req.params.empresa;
    const empresaResolvida = await validarAcessoEmpresa(req, empresa);
    ...
    res.status(500).send('Erro ao gerar relatório de contas a receber');
  }
});

app.get('/relatorios/financeiro/contas-pagar/:empresa', auth, async (req, res) => {
  try {
    const empresa = req.params.empresa;
    const empresaResolvida = await validarAcessoEmpresa(req, empresa);
    ...
    res.status(500).send('Erro ao gerar relatório de contas a pagar');
  }
});

app.get('/relatorios/financeiro/fluxo-caixa/:empresa', auth, async (req, res) => {
  try {
    const empresa = req.params.empresa;
    const empresaResolvida = await validarAcessoEmpresa(req, empresa);
    ...
    res.status(500).send('Erro ao gerar relatório de fluxo de caixa');
  }
});
